...
/**
 *  \file       seguro_document.php
 *  \ingroup    frota
 *  \brief      Tab for documents linked to Seguro
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->frota->multidir_output[$object->entity ? $object->entity : $conf->entity]."/seguro/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('frota', 'seguro', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("Seguro")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("Seguro"), -1, $object->picto);
...
$modulepart = 'frota';
$param = '&id='.$object->id;
//$relativepathwithnofile='seguro/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'seguro/'.dol_sanitizeFileName($object->ref).'/';
