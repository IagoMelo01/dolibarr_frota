...
/**
 *   	\file       seguro_list.php
 *		\ingroup    frota
 *		\brief      List page for seguro
 */
...
require_once __DIR__.'/class/seguro.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('frota', 'seguro', 'write');
	$permissiontodelete = $user->hasRight('frota', 'seguro', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
$title = $langs->trans("Seguros");
//$help_url = "EN:Module_Seguro|FR:Module_Seguro_FR|ES:Módulo_Seguro";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/frota/seguro_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/frota/seguro_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendSeguroRef";
$modelmail = "seguro";
$objecttmp = new Seguro($db);
