...
/**
 *   	\file       abastecimento_list.php
 *		\ingroup    frota
 *		\brief      List page for abastecimento
 */
...
require_once __DIR__.'/class/abastecimento.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('frota', 'abastecimento', 'write');
	$permissiontodelete = $user->hasRight('frota', 'abastecimento', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Abastecimento';
	$objectlabel = 'Abastecimento';
...
$title = $langs->trans("Abastecimentos");
//$help_url = "EN:Module_Abastecimento|FR:Module_Abastecimento_FR|ES:Módulo_Abastecimento";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/frota/abastecimento_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-frota page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/frota/abastecimento_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendAbastecimentoRef";
$modelmail = "abastecimento";
$objecttmp = new Abastecimento($db);
