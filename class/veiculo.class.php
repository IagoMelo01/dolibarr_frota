...
/**
 * \file        class/veiculo.class.php
 * \ingroup     frota
 * \brief       This file is a CRUD class file for Veiculo (Create/Read/Update/Delete)
 */
 ...
 * Class for Veiculo
 */
class Veiculo extends CommonObject
{
	/**
	...
	//  */
	// public $table_element_line = 'frota_veiculoline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_veiculo';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Veiculoline';
	...
	//  */
	// protected $childtablesoncascade = array('frota_veiculodet');

	// /**
	//  * @var VeiculoLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->frota->veiculo->read) {
	...
	protected function set_ref(int $obj_id){
		...
		$rid = $v->rowid;
		$fab = $v->fields['fabricante']['arrayofkeyval'][$v->fabricante];
		$r = '';
		if($rid < 10){
			$r .= '00';
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('frota','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && !empty($user->rights->frota->veiculo->veiculo_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			if (!$error && !$notrigger) {
				// Call trigger
				$result = $this->call_trigger('VEICULO_VALIDATE', $user);
				if ($result < 0) {
					$error++;
				}
				// End call triggers
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'veiculo/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'veiculo/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
				...
				$dirsource = $conf->frota->dir_output.'/veiculo/'.$oldref;
				$dirdest = $conf->frota->dir_output.'/veiculo/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->frota->dir_output.'/veiculo/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'VEICULO_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'VEICULO_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'VEICULO_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$label = img_picto('', $this->picto).' <u>'.$langs->trans("Veiculo").'</u>';
		...
		$url = dol_buildpath('/frota/veiculo_card.php', 1).'?id='.$this->id;
	...
	*/
	...
	public function getLinesArray()
	{
		...
		$objectline = new VeiculoLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_veiculo = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (getDolGlobalString('FROTA_VEICULO_ADDON')=='') {
			$conf->global->FROTA_VEICULO_ADDON = 'mod_veiculo_standard';
		}

		if (getDolGlobalString('FROTA_VEICULO_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('FROTA_VEICULO_ADDON').".php";
			$classname = getDolGlobalString('FROTA_VEICULO_ADDON');
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_veiculo';

			if (!empty($this->model_pdf)) {
				$modele = $this->model_pdf;
			} elseif (getDolGlobalString('VEICULO_ADDON_PDF')) {
				$modele = getDolGlobalString('VEICULO_ADDON_PDF');
			}
		}
...
/**
 * Class VeiculoLine. You can also remove this and generate a CRUD class for lines objects.
 */
class VeiculoLine extends CommonObjectLine
{
	// To complete with content of an object VeiculoLine
	// We should have a field rowid, fk_veiculo and position
