...
class Veiculo extends CommonObject
{
	...
	public $fields=array(
		...
		'amount' => array('type'=>'price', 'label'=>'Amount', 'enabled'=>'1', 'position'=>40, 'notnull'=>0, 'visible'=>1, 'default'=>'null', 'isameasure'=>'1', 'help'=>"Valor do veículo", 'validate'=>'1',),
	...
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Veiculo', 'fk_element'=>'fk_veiculo'));
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('frota', 'veiculo', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('frota', 'veiculo_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'veiculo/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'veiculo/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowVeiculo")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Veiculo").'</u>';
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('FROTA_MYOBJECT_ADDON')) {
			$conf->global->FROTA_MYOBJECT_ADDON = 'mod_veiculo_standard';
		}
