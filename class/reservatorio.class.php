...
/**
 * \file        class/reservatorio.class.php
 * \ingroup     frota
 * \brief       This file is a CRUD class file for Reservatorio (Create/Read/Update/Delete)
 */
...
/**
 * Class for Reservatorio
 */
class Reservatorio extends CommonObject
{
	/**
	...
	public $fields=array(
		...
		'capacidade' => array('type'=>'integer', 'label'=>'Capacidade do reservatório (litros)', 'enabled'=>'1', 'position'=>50, 'notnull'=>1, 'visible'=>-1,),
		'nivel' => array('type'=>'integer', 'label'=>'Nível atual (aproximado)', 'enabled'=>'1', 'position'=>50, 'notnull'=>1, 'visible'=>-1,),
		'tipo' => array('type'=>'integer', 'label'=>'Combustível', 'enabled'=>'1', 'position'=>50, 'notnull'=>1, 'visible'=>-1, 'arrayofkeyval'=>array('0'=>'Diesel', '1'=>'Gasolina', '2'=>'Etanol', '3'=>'Outro'),),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// public $table_element_line = 'frota_reservatorioline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_reservatorio';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Reservatorioline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Reservatorio', 'fk_element'=>'fk_reservatorio'));
	...
	//  */
	// protected $childtablesoncascade = array('frota_reservatoriodet');

	// /**
	//  * @var ReservatorioLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('frota', 'reservatorio', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('frota', 'reservatorio', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('frota', 'reservatorio_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'reservatorio/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'reservatorio/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'reservatorio/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'reservatorio/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->frota->dir_output.'/reservatorio/'.$oldref;
				$dirdest = $conf->frota->dir_output.'/reservatorio/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->frota->dir_output.'/reservatorio/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowReservatorio")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Reservatorio").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/frota/reservatorio_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowReservatorio");
	...
	public function getLinesArray()
	{
		...
		$objectline = new ReservatorioLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_reservatorio = '.((int) $this->id)));
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'pdf_standard_reservatorio';
...
/**
 * Class ReservatorioLine. You can also remove this and generate a CRUD class for lines objects.
 */
class ReservatorioLine extends CommonObjectLine
{
	// To complete with content of an object ReservatorioLine
	// We should have a field rowid, fk_reservatorio and position
