...
/**
 * \file        class/manutencao.class.php
 * \ingroup     frota
 * \brief       This file is a CRUD class file for Manutencao (Create/Read/Update/Delete)
 */
...
/**
 * Class for Manutencao
 */
class Manutencao extends CommonObject
{
	/**
	...
	//  */
	// public $table_element_line = 'frota_manutencaoline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_manutencao';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Manutencaoline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Manutencao', 'fk_element'=>'fk_manutencao'));
	...
	//  */
	// protected $childtablesoncascade = array('frota_manutencaodet');

	// /**
	//  * @var ManutencaoLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('frota', 'manutencao', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('frota', 'manutencao_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'manutencao/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'manutencao/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'manutencao/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'manutencao/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->frota->dir_output.'/manutencao/'.$oldref;
				$dirdest = $conf->frota->dir_output.'/manutencao/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->frota->dir_output.'/manutencao/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowManutencao")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Manutencao").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/frota/manutencao_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowManutencao");
	...
	public function getLinesArray()
	{
		...
		$objectline = new ManutencaoLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_manutencao = '.((int) $this->id)));
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('FROTA_MYOBJECT_ADDON')) {
			$conf->global->FROTA_MYOBJECT_ADDON = 'mod_manutencao_standard';
		}
...
/**
 * Class ManutencaoLine. You can also remove this and generate a CRUD class for lines objects.
 */
class ManutencaoLine extends CommonObjectLine
{
	// To complete with content of an object ManutencaoLine
	// We should have a field rowid, fk_manutencao and position
