...
/**
 * \file        class/compracombustivel.class.php
 * \ingroup     frota
 * \brief       This file is a CRUD class file for CompraCombustivel (Create/Read/Update/Delete)
 */
...
/**
 * Class for CompraCombustivel
 */
class CompraCombustivel extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'compracombustivel';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'frota_compracombustivel';
	...
	public $fields=array(
		...
		'amount' => array('type'=>'price', 'label'=>'Preço por litro', 'enabled'=>'1', 'position'=>40, 'notnull'=>0, 'visible'=>1, 'default'=>'null', 'isameasure'=>'1', 'help'=>"Help text for amount", 'validate'=>'1',),
		...
		'fk_reservatorio' => array('type'=>'integer:Reservatorio:frota/class/reservatorio.class.php', 'label'=>'Reservatório de destino', 'enabled'=>'1', 'position'=>50, 'notnull'=>1, 'visible'=>-1,),
	);
	public $rowid;
	public $ref;
	public $label;
	...
	//  */
	// public $table_element_line = 'frota_compracombustivelline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_compracombustivel';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'CompraCombustivelline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'CompraCombustivel', 'fk_element'=>'fk_compracombustivel'));
	...
	//  */
	// protected $childtablesoncascade = array('frota_compracombustiveldet');

	// /**
	//  * @var CompraCombustivelLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('frota', 'compracombustivel', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('frota', 'compracombustivel', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('frota', 'compracombustivel_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'compracombustivel/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'compracombustivel/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'compracombustivel/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'compracombustivel/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->frota->dir_output.'/compracombustivel/'.$oldref;
				$dirdest = $conf->frota->dir_output.'/compracombustivel/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->frota->dir_output.'/compracombustivel/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowCompraCombustivel")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("CompraCombustivel").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/frota/compracombustivel_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowCompraCombustivel");
	...
	public function getLinesArray()
	{
		...
		$objectline = new CompraCombustivelLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, array('customsql'=>'fk_compracombustivel = '.((int) $this->id)));
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_compracombustivel';
...
/**
 * Class CompraCombustivelLine. You can also remove this and generate a CRUD class for lines objects.
 */
class CompraCombustivelLine extends CommonObjectLine
{
	// To complete with content of an object CompraCombustivelLine
	// We should have a field rowid, fk_compracombustivel and position
