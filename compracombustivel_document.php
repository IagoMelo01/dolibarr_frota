...
/**
 *  \file       compracombustivel_document.php
 *  \ingroup    frota
 *  \brief      Tab for documents linked to CompraCombustivel
 */
...
if ($id > 0 || !empty($ref)) {
	$upload_dir = $conf->frota->multidir_output[$object->entity ? $object->entity : $conf->entity]."/compracombustivel/".get_exdir(0, 0, 0, 1, $object);
}
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd  = $user->hasRight('frota', 'compracombustivel', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_linkedfiles.inc.php
} else {
	$permissiontoread = 1;
	$permissiontoadd  = 1;
}
...
// Header
// ------
$title = $langs->trans("CompraCombustivel")." - ".$langs->trans("Files");
...
print dol_get_fiche_head($head, 'document', $langs->trans("CompraCombustivel"), -1, $object->picto);
...
$modulepart = 'frota';
$param = '&id='.$object->id;
//$relativepathwithnofile='compracombustivel/' . dol_sanitizeFileName($object->id).'/';
$relativepathwithnofile = 'compracombustivel/'.dol_sanitizeFileName($object->ref).'/';
