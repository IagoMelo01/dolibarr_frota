...
/**
 *   	\file       implemento_list.php
 *		\ingroup    frota
 *		\brief      List page for implemento
 */
...
// load frota libraries
require_once __DIR__.'/class/implemento.class.php';
...
$hookmanager->initHooks(array('implementolist')); // Note that conf->hooks_modules contains array
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('frota', 'implemento', 'write');
	$permissiontodelete = $user->hasRight('frota', 'implemento', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("frota")) {
	accessforbidden('Module frota not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Implemento';
	$objectlabel = 'Implemento';
	$uploaddir = $conf->frota->dir_output;
...
//$help_url = "EN:Module_Implemento|FR:Module_Implemento_FR|ES:Módulo_Implemento";
$help_url = '';
$title = $langs->trans("Implementos");
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/frota/implemento_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/frota/implemento_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendImplementoRef";
$modelmail = "implemento";
$objecttmp = new Implemento($db);
...
if (in_array('builddoc', $arrayofmassactions) && ($nbtotalofrecords === '' || $nbtotalofrecords)) {
	...
	print $formfile->showdocuments('massfilesarea_frota', '', $filedir, $urlsource, 0, $delallowed, '', 1, 1, 0, 48, 1, $param, $title, '', '', '', null, $hidegeneratedfilelistifempty);
}
